...
for (i in 1:length(samplesForMM)){
  ...
  MMdata0 <- FSfinal@frames[[sample0]]@exprs[, c('log_MX')] %>%
    as.matrix()%>%
    .[!is.na(.)]
...
MXMMlambdas<-data.frame('high'=unlist(highLam),'low'=unlist(lowLam),name=unlist(nameList))
write.csv(MXMMlambdas,('Lambdas/MX.csv'))
...
for (i in 1:length(samplesForNorm)){
  ...
  Normdata0 <- FSfinal@frames[[sample0]]@exprs[, c('log_MX')] %>%
    as.matrix()%>%
    .[!is.na(.)]
...
for(i in 1:3){
  ...
  ggsave(filename=paste0('./Figs/MX/linear blanking/Mixed model graph replicate',i,'.tiff'),plot=plot0,width=300,height=190,units='mm')
}
...
write.csv(MMparamsWT,'MX parameters/MMparams_MX.csv')
MMCombinedMean<-lapply(MMDataList,mean)
names(MMCombinedMean)<- gsub("-k", "no ko143", names(MMCombinedMean))
write.csv(MMCombinedMean,'MX parameters/MM_combinedMean_MX.csv')


samplesNormAll<-sampleNames(FSfinal)[!(sampleNames(FSfinal)%like% 'WT MX -k' | sampleNames(FSfinal) %like% 'unstained')]
NormListAll<-list()
...
write.csv(NormParams,'MX parameters/NormParams_MX.csv')
