...
for (i in 1:length(samplesForNorm)){
  ...
  Normdata0 <- FSfinal@frames[[sample0]]@exprs[, c('log_CFSE_iso')] %>%
    as.matrix()%>%
    .[!is.na(.)]
...
for (i in 1:length(samplesForMM)){
  ...
  MMdata0 <- FSfinal@frames[[sample0]]@exprs[, c('log_CFSE_iso')] %>%
    as.matrix()%>%
    .[!is.na(.)]
  MM0 <- normalmixEM(x = MMdata0,
                     ...
                     epsilon = 1e-16,
                     maxrestarts=1000,
                     mean.constr = c(NA,NormList[[i]]$estimate['mean']),
                     k = 2)
  MMList[[i]]<-MM0
...
CFSEMMlambdas<-data.frame('high'=unlist(highLam),'low'=unlist(lowLam),name=unlist(nameList))
write.csv(CFSEMMlambdas,('Lambdas/CFSE_iso.csv'))
...
for(i in 1:length(MMDataList)){
  plot0<-ggplot() +
    ...
    xlab('log10(CFSE fluorescence), isosbestic wavelength') +
  ...
  ggsave(filename=paste0('./Figs/CFSE/linear blanking/CFSE_iso mixed model graph replicate',i,'.tiff'),plot=plot0,width=300,height=190,units='mm')
}
...
write.csv(MMparamsWT,'./CFSE parameters/MMparams_CFSE_iso.csv')
MMCombinedMean<-lapply(MMDataList,mean)
names(MMCombinedMean)<- gsub("-k", "no ko143", names(MMCombinedMean))
write.csv(MMCombinedMean,'./CFSE parameters/MM_combinedMean_CFSE_iso.csv')


samplesNormAll<-sampleNames(FSfinal)[(!( sampleNames(FSfinal)%like% 'unstained' | sampleNames(FSfinal)%like% 'WT test -k' ))&
                                       sampleNames(FSfinal)%like% 'test']
NormListAll<-list()
...
write.csv(NormParams,'./CFSE parameters/NormParams_CFSE_iso.csv')
