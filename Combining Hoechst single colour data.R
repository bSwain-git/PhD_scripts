...
files.list
data_Hoechst <- lapply(files.list[files.list %like% 'Hoechst'],
                       function(x) {
                         readRDS(paste0(directory0, x))
                       })

#One dataset had no pH annotation (was done at normal buffer pH == 7.4)
pData(data_Hoechst[[3]])$pH <- 7.4

##annotate each with replicate number:
for (i in 1:length(data_Hoechst)) {
  sampleNames(data_Hoechst[[i]]) <-
    sampleNames(data_Hoechst[[i]]) %>%
    lapply(function(name0) {
      paste(name0, 'rep', i)
    })
  
  pData(data_Hoechst[[i]])$name <-
    pData(data_Hoechst[[i]])$name %>%
    lapply(function(name0) {
      paste(name0, 'rep', i)
    })
  
  pData(data_Hoechst[[i]])$replicate <- as.factor(i)
  class(pData(data_Hoechst[[1]])$name) <- 'character'
}

#combine the sets together
Hoechst_combined <- rbind2(data_Hoechst[[1]], data_Hoechst[[2]])
Hoechst_combined <- rbind2(Hoechst_combined, data_Hoechst[[3]])
pData(Hoechst_combined)
FS <- Hoechst_combined
...
ggplot(data = FSfiltered[(pData(FSfiltered)$replicate==2) & 
                           pData(FSfiltered)$dye=='unstained'], aes(x =Hoechst, fill = dye)) +
  geom_density(alpha = 0.5) +
  facet_grid(~ cellType  , scales = 'fixed') +
  # scale_x_log10() +
  xlim(-1500,2000)+
  theme(axis.text.y = element_blank(), axis.ticks.y = element_blank()) +
  xlab('Hoechst fluorescence') +
...
for (i in 1:length(unstainedConditions)){
  ...
  unstainedVal0<-
    FS_BGsub@frames[[unstained0]]@exprs[, c('Hoechst')] %>%
    as.matrix() %>%
    median(na.rm = T)
  for (j in 1:length(toBlank0)){
    currentlyBlanking<-toBlank0[j]
    FS_BGsub@frames[[currentlyBlanking]]@exprs[, c('Hoechst')]<-
      FS_BGsub@frames[[currentlyBlanking]]@exprs[, c('Hoechst')]-unstainedVal0
  }
  
}

FS_BGsub <-
  flowCore::transform(FS_BGsub, 'log_Hoechst' = log10(`Hoechst`))


#plot blanked data:
ggplot(data = FS_BGsub[!pData(FS_BGsub)$dye=='unstained'&
                         pData(FS_BGsub)$replicate == 2], aes(x = log_Hoechst, fill = inhibitor)) +
  geom_density(alpha = 0.5) +
  facet_grid(~ cellType  , scales = 'fixed') +
  # scale_x_log10() +
  xlim(2.5,6)+
...
saveRDS(FS_BGsub,'FS_final_Hoechst.rds')
